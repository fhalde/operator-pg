...
import cli.strimzi.kafka.v1beta2.kafkaconnectspec.template.initcontainer.Env;
import java.util.LinkedHashMap;
import java.util.function.Predicate;
import cli.strimzi.kafka.v1beta2.kafkaconnectspec.template.initcontainer.SecurityContextBuilder;
import io.fabric8.kubernetes.api.builder.BaseFluent;
import java.util.Iterator;
import java.util.List;
import cli.strimzi.kafka.v1beta2.kafkaconnectspec.template.initcontainer.EnvFluent;
import cli.strimzi.kafka.v1beta2.kafkaconnectspec.template.initcontainer.EnvBuilder;
import cli.strimzi.kafka.v1beta2.kafkaconnectspec.template.initcontainer.SecurityContext;
import cli.strimzi.kafka.v1beta2.kafkaconnectspec.template.initcontainer.SecurityContextFluent;
...
public class InitContainerFluent<A extends InitContainerFluent<A>> extends BaseFluent<A>{
  public InitContainerFluent() {
  }
  
  public InitContainerFluent(InitContainer instance) {
  ...
  protected void copyInstance(InitContainer instance) {
    instance = (instance != null ? instance : new InitContainer());
    if (instance != null) {
  ...
  public A addToEnv(cli.strimzi.kafka.v1beta2.kafkaconnectspec.template.initcontainer.Env... items) {
    if (this.env == null) {this.env = new ArrayList<EnvBuilder>();}
  ...
  public A addAllToInitcontainerEnv(Collection<Env> items) {
    if (this.env == null) {this.env = new ArrayList<EnvBuilder>();}
    for (Env item : items) {EnvBuilder builder = new EnvBuilder(item);_visitables.get("env").add(builder);this.env.add(builder);} return (A)this;
  }
  
  public A removeFromEnv(cli.strimzi.kafka.v1beta2.kafkaconnectspec.template.initcontainer.Env... items) {
  ...
  public A removeAllFromInitcontainerEnv(Collection<Env> items) {
    if (this.env == null) return (A)this;
    for (Env item : items) {EnvBuilder builder = new EnvBuilder(item);_visitables.get("env").remove(builder); this.env.remove(builder);} return (A)this;
  }
  
  public A removeMatchingFromInitcontainerEnv(Predicate<EnvBuilder> predicate) {
  ...
  public A withEnv(cli.strimzi.kafka.v1beta2.kafkaconnectspec.template.initcontainer.Env... env) {
    if (this.env != null) {
        this.env.clear();
  ...
  public SecurityContextNested<A> editInitcontainerSecurityContext() {
  ...
  public boolean equals(Object o) {
    ...
    InitContainerFluent that = (InitContainerFluent) o;
  ...
  public class EnvNested<N> extends EnvFluent<EnvNested<N>> implements Nested<N>{
    ...
    public N and() {
      return (N) InitContainerFluent.this.setToEnv(index,builder.build());
    }
    
    public N endInitcontainerEnv() {
      return and();
    }
  ...
  public class SecurityContextNested<N> extends SecurityContextFluent<SecurityContextNested<N>> implements Nested<N>{
    ...
    public N and() {
      return (N) InitContainerFluent.this.withSecurityContext(builder.build());
    }
    
    public N endInitcontainerSecurityContext() {
      return and();
    }
