...
import io.strimzi.kafka.v1beta2.kafkaconnectspec.build.plugins.ArtifactsBuilder;
import java.lang.SuppressWarnings;
import io.fabric8.kubernetes.api.builder.Nested;
import java.util.ArrayList;
import java.lang.String;
import io.strimzi.kafka.v1beta2.kafkaconnectspec.build.plugins.ArtifactsFluent;
import java.util.LinkedHashMap;
import java.util.function.Predicate;
import io.strimzi.kafka.v1beta2.kafkaconnectspec.build.plugins.Artifacts;
...
public class PluginsFluent<A extends PluginsFluent<A>> extends BaseFluent<A>{
  ...
  public A addToArtifacts(io.strimzi.kafka.v1beta2.kafkaconnectspec.build.plugins.Artifacts... items) {
    if (this.artifacts == null) {this.artifacts = new ArrayList<ArtifactsBuilder>();}
  ...
  public A addAllToIoArtifacts(Collection<Artifacts> items) {
    if (this.artifacts == null) {this.artifacts = new ArrayList<ArtifactsBuilder>();}
    for (Artifacts item : items) {ArtifactsBuilder builder = new ArtifactsBuilder(item);_visitables.get("artifacts").add(builder);this.artifacts.add(builder);} return (A)this;
  }
  
  public A removeFromArtifacts(io.strimzi.kafka.v1beta2.kafkaconnectspec.build.plugins.Artifacts... items) {
  ...
  public A removeAllFromIoArtifacts(Collection<Artifacts> items) {
    if (this.artifacts == null) return (A)this;
    for (Artifacts item : items) {ArtifactsBuilder builder = new ArtifactsBuilder(item);_visitables.get("artifacts").remove(builder); this.artifacts.remove(builder);} return (A)this;
  }
  
  public A removeMatchingFromIoArtifacts(Predicate<ArtifactsBuilder> predicate) {
  ...
  public A withArtifacts(io.strimzi.kafka.v1beta2.kafkaconnectspec.build.plugins.Artifacts... artifacts) {
    if (this.artifacts != null) {
